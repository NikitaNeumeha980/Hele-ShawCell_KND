...
translationNbTimes = int(sys.argv[5])

numberOfSegmentsAlongChannelLength0 = 300

backgroundMeshBoxLength = translationNbTimes*channelLength
...
channelProfile1D = geompy.MakeCurveParametric("0.5 *" + str(Rmax + Rmin) + "- 0.5 *" + str(Rmax - Rmin) + "* cos(2 * pi * t /" + str(channelLength) + ")", "0", "t", 0, translationNbTimes*channelLength, 100*translationNbTimes, GEOM.Interpolation, True)

channelWall = geompy.MakePrismVecH(channelProfile1D, OY, backgroundMeshBoxDepth)

sk = geompy.Sketcher3D()
sk.addPointsAbsolute(Rmin, 0, 0)
sk.addPointsAbsolute(0, 0, 0)
sk.addPointsAbsolute(0, 0, translationNbTimes*channelLength)
sk.addPointsAbsolute(Rmin, 0, translationNbTimes*channelLength)
...
geompy.addToStudy( planeChannel3D, 'planeChannel3DRmax' + str(Rmax) + 'Rmin' + str(Rmin) + 'channelLength' + str(channelLength) + 'translationNbTimes' + str(translationNbTimes))
...
geompy.ExportSTL(channelWall, ABSOLUTE_CASE_PATH + "/channelWall.stl", True, 1e-5, True)
geompy.ExportSTL(planeChannel3D, ABSOLUTE_CASE_PATH + "/planeChannel3D.stl", True, 1e-5, True)
...
#Sub-mesh on width
regular1DAlongWidth = backgroundMeshBoxMesh.Segment(geom=edgeForSubMeshOnWidth)
numberOfSegmentsAlongWidth = regular1DAlongWidth.NumberOfSegments(
    max(
        [
...
propagationOf1DHypAlongWidth = regular1DAlongWidth.Propagation()
...
smesh.SetName(backgroundMeshBoxMesh.GetMesh(), 'backgroundMeshBoxMeshRmax' + str(Rmax) + 'channelLength' + str(channelLength) + 'translationNbTimes' + str(translationNbTimes))
...
inlet_1 = backgroundMeshBoxMesh.GroupOnFilter( SMESH.FACE, 'inlet_1', aFilter )
...
aFilter = smesh.GetFilterFromCriteria(aCriteria)
aFilter.SetMesh(backgroundMeshBoxMesh.GetMesh())
outlet_1 = backgroundMeshBoxMesh.GroupOnFilter( SMESH.FACE, 'outlet_1', aFilter )
...
#setFieldsRegions = open(ABSOLUTE_CASE_PATH + "/setFieldsRegions", "w", 0)
#setFieldsRegions.write(
        #"sphereToCell
        #{
            #centre (" + str(0) + " " + str(0) + " " str(0.5*channelLength) + ");"
            #+ "radius" + str(0.5 * backgroundMeshBoxDepth) + " " + str(0.5 * channelLength) + 
        #"}"
    #)
