...
#Calculation
foamDictionary  -entry "numberOfSubdomains" -set "62" system/decomposeParDict
foamDictionary  -entry "method" -set "scotch" system/decomposeParDict
runApplication  ideasUnvToFoam splittedChannelMesh.unv
# runApplication  checkMesh -allGeometry -allTopology
# mv log.checkMesh log.originMesh.checkMesh
runApplication  polyDualMesh -overwrite 30
# runApplication  checkMesh -allGeometry -allTopology
# mv log.checkMesh log.polyDualMesh.checkMesh
runApplication  topoSet
cp -r 0.orig 0
runApplication  decomposePar
...
mv log.interFoam log.interFoam0
...
for d in processor*/ ; do
    ...
    foamDictionary -entry boundaryField.right -set\
        "{  
            ...
            volumetricFlowRate  4e-11;
            extrapolateProfile  yes;
            value               uniform (0 0 0);
        }
        "\
        ${d}/$(foamListTimes -processor -latestTime | tail -1)/U
    foamDictionary -entry boundaryField.left.type -set "zeroGradient" ${d}/$(foamListTimes -processor -latestTime | tail -1)/p_rgh
