...
gasketDXDYDZR_ =\
    [
        voxelSizeDXDYDZ_[0]*(Decimal('50.0')/voxelSizeDXDYDZ_[0]).quantize(Decimal('1'), ROUND_CEILING),
        voxelSizeDXDYDZ_[1]*(Decimal('50.0')/voxelSizeDXDYDZ_[1]).quantize(Decimal('1'), ROUND_CEILING),
...
borderDXDYDZ_ =\
    [
        voxelSizeDXDYDZ_[0]*(Decimal('5.0')/voxelSizeDXDYDZ_[0]).quantize(Decimal('1'), ROUND_CEILING),
        voxelSizeDXDYDZ_[1]*(Decimal('5.0')/voxelSizeDXDYDZ_[1]).quantize(Decimal('1'), ROUND_CEILING),
        voxelSizeDXDYDZ_[2]*(Decimal('5.0')/voxelSizeDXDYDZ_[2]).quantize(Decimal('1'), ROUND_CEILING),
    ]

wallThikness_ = voxelSizeDXDYDZ_[0]*(Decimal('2.0')/voxelSizeDXDYDZ_[0]).quantize(Decimal('1'), ROUND_CEILING)
    ...
    geompy.MakeCutList(
        geompy.TranslateDXDYDZ(
            geompy.MakeBoxDXDYDZ(
                float(gasketDXDYDZR_[0] + borderDXDYDZ_[0]),
                float(gasketDXDYDZR_[1] + borderDXDYDZ_[1]),
                float(borderDXDYDZ_[2])
            ),
            -float((gasketDXDYDZR_[0] + borderDXDYDZ_[0])/2),
            -float((gasketDXDYDZR_[1] + borderDXDYDZ_[1])/2),
            0
        ),
        [
            geompy.TranslateDXDYDZ(
                geompy.Rotate(
                    geompy.Rotate(
                        geompy.MakeCylinderRH(float(gasketDXDYDZR_[3]), float(gasketDXDYDZR_[0])),
                        OZ,
                        math.pi
                    ),
                    OY,
                ...
                -float(gasketDXDYDZR_[0]/2),
                0,
                float(((gasketDXDYDZR_[3])**2 - (gasketDXDYDZR_[1]/2)**2).sqrt() + borderDXDYDZ_[2])
            ),
            geompy.MakeCutList(
                geompy.TranslateDXDYDZ(
                    geompy.MakeBoxDXDYDZ(
                        float(gasketDXDYDZR_[0]),
                        float(gasketDXDYDZR_[1]),
                        float(borderDXDYDZ_[2])
                    ),
                    -float(gasketDXDYDZR_[0]/2),
                    -float(gasketDXDYDZR_[1]/2),
                    0
                ),
                [
                    geompy.TranslateDXDYDZ(
                        ...
                        -float(gasketDXDYDZR_[0]/2),
                        0,
                        float(((gasketDXDYDZR_[3])**2 - (gasketDXDYDZR_[1]/2)**2).sqrt() + (borderDXDYDZ_[2] - wallThikness_))
                    )
                ],
                True
            )
...
stiffnerRib =\
    geompy.TranslateDXDYDZ(
        geompy.MakeMultiTranslation1D(
            ...
            OX,
            float(gasketDXDYDZR_[0]/stiffnerRibNbTimes_[0]),
            int(stiffnerRibNbTimes_[0])
        ),
        -float(gasketDXDYDZR_[0]/stiffnerRibNbTimes_[0]),
        -float(gasketDXDYDZR_[1]/2),
        0,
...
geompy.addToStudy( castingMold, 'castingMold' )
geompy.addToStudy( stiffnerRib, 'stiffnerRib' )
