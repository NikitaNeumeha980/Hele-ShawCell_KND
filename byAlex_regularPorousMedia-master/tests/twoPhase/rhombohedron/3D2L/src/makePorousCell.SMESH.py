...
globalLocalLengthSize = 0.1 * max([geompy.BasicProperties(allEdgesWithFillet[edgeIndex])[0] for edgeIndex in range(len(allEdgesWithFillet))])
localLengthSize = 0.07 * min([geompy.BasicProperties(allEdgesWithoutFillet[edgeIndex])[0] for edgeIndex in range(len(allEdgesWithoutFillet))])

NETGEN_2D_MaxSize = 1.0 * localLengthSize
NETGEN_2D_MinSize = 1.0 * localLengthSize
NETGEN_2D_MaxSize_1 = 1.0 * localLengthSize
NETGEN_2D_MinSize_1 = 1.0 * localLengthSize
NETGEN_2D_QuadAllowed = 0
growthRate2D = 0.1

#NETGEN_3D_MaxSize = 0.1 * geompy.BasicProperties(onePore)[2] ** (1.0 / 3.0)
#NETGEN_3D_MinSize = 0.05 * geompy.BasicProperties(onePore)[2] ** (1.0 / 3.0)
NETGEN_3D_MaxSize = 1.2 * localLengthSize
NETGEN_3D_MinSize = 1.0 * localLengthSize
growthRate3D = 0.1

#viscousLayersSize3D = 10.0 * 1e-3 * onePoreCellSize
viscousLayersSize3D = onePoreCellSize / increasingScaleFactor
viscousLayersNumber = 4
viscousLayersGrowth = 1.0
####################################################
...
if(
    ...
    globalNETGEN1D2DParameters.SetNbSegPerRadius( 6 )
...
globalNETGEN3DParameters.SetMaxSize( NETGEN_3D_MaxSize )
globalNETGEN3DParameters.SetMinSize( NETGEN_3D_MinSize )
globalNETGEN3DParameters.SetOptimize( 1 )
globalNETGEN3DParameters.SetFineness( 5 )
globalNETGEN3DParameters.SetGrowthRate( growthRate3D )
globalNETGEN3DParameters.SetUseSurfaceCurvature( 1 )
...
if (
    ...
    viscousLayers3D = gloabalNETGEN3D.ViscousLayers(viscousLayersSize3D,viscousLayersNumber,viscousLayersGrowth,allFacesIDsWithoutViscousLayers,1,StdMeshersBuilder.FACE_OFFSET)
...
aCriteria = []
aCriterion = smesh.GetCriterion(SMESH.VOLUME,SMESH.FT_LyingOnGeom,SMESH.FT_Undefined,skeletonWall,SMESH.FT_Undefined,SMESH.FT_Undefined,1.1 * viscousLayersSize3D)
aCriteria.append(aCriterion)
aFilter_1 = smesh.GetFilterFromCriteria(aCriteria)
aFilter_1.SetMesh(porousCellMesh.GetMesh())
film = porousCellMesh.GroupOnFilter( SMESH.VOLUME, 'film', aFilter_1 )
