...
maxDropletRadius=$(echo "scale=${scaleForFolderNames}; 0.9 * ${unitCellSize};" | bc -l)
...
#         mkdir -p firstDirection secondDirection thirdDirection
        ...
        centreOfMassX=$(sed 's/.*(\(.*\)).*/\1/' centreOfMass | sed 's/^[ \t]*//;s/[ \t]*$//g' | sed 's/[ ]\+/\n/g' | sed -n '1p')
        centreOfMassY=$(sed 's/.*(\(.*\)).*/\1/' centreOfMass | sed 's/^[ \t]*//;s/[ \t]*$//g' | sed 's/[ ]\+/\n/g' | sed -n '2p')
        centreOfMassZ=$(sed 's/.*(\(.*\)).*/\1/' centreOfMass | sed 's/^[ \t]*//;s/[ \t]*$//g' | sed 's/[ ]\+/\n/g' | sed -n '3p')

        echo "centre ("\
            $(echo "scale=${scaleForCalculations}; ${scaledScaleFactor}*${centreOfMassX};" | bc -l) \
            $(echo "scale=${scaleForCalculations}; ${scaledScaleFactor}*${centreOfMassY};" | bc -l) \
            $(echo "scale=${scaleForCalculations}; ${scaledScaleFactor}*${centreOfMassZ};" | bc -l)");" > centreOfMassScaled
