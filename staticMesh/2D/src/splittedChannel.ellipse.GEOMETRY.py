...
with open(absoluteCasePath_ + "/forTopoSetBox", 'w') as forTopoSetBox:
    forTopoSetBox.write(
        "box ("
        + str(-(max(a1_,a2_)+0.5*blobSeparatingDistance_)) + " " + str(-b2_) + " " + str(-depth_) +
        ")("
        + str(max(a1_,a2_)+0.5*blobSeparatingDistance_) + " " + str(b1_) + " " + str(depth_)+
        ");"
    )
...
with open(absoluteCasePath_ + "/forSetFieldsBox", 'w') as forSetFieldsBox:
    forSetFieldsBox.write(
        str(max(a1_,a2_)+0.5*blobSeparatingDistance_ + 200)
    )
...
with open(absoluteCasePath_ + "/system/snappyHexMeshSrc/refinementBox", 'w') as refinementBox:
    refinementBox.write(
            "min (" + str(-(max(a1_,a2_)+0.5*blobSeparatingDistance_)) + " " + str(-1e6) + " " + str(-depth_) + ");" + "\n" +
            "max (" + str(max(a1_,a2_)+0.5*blobSeparatingDistance_) + " " + str(1e6) + " " + str(depth_)+ ");"
    )
...
#backFace = geompy.MakeFace(backContour, 1)
...
#frontFace = geompy.MakeFace(frontContour, 1)
