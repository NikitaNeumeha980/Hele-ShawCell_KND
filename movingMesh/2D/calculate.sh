...
runApplication -o surfaceFeatures -dict system/snappyHexMeshSrc/
runApplication -o surfaceFeatureConvert constant/triSurface/*.eMesh constant/triSurface/edges.vtk

#Preparation background mesh for snappyHexMesh
runApplication  blockMesh

#Run snappyHexMesh
runApplication -o foamDictionary -entry numberOfSubdomains -set 4 system/decomposeParDict
runApplication -a foamDictionary -entry method -set scotch system/decomposeParDict
runApplication -o decomposePar -copyZero
runParallel -o    snappyHexMesh -overwrite -dict system/snappyHexMeshSrc/
runParallel -a    foamDictionary constant/polyMesh/boundary -entry entry0/back/type -set symmetry
runParallel -a    foamDictionary constant/polyMesh/boundary -entry entry0/front/type -set symmetry
runParallel -o    checkMesh -allTopology -allGeometry

#Run the topoSet utility for creation viscous sub-layer
runParallel -o    topoSet
...
runParallel -o    transformPoints "scale=(${scaleSize} ${scaleSize} ${scaleSize})"

runApplication -a foamDictionary system/controlDict -set\
    "
        application=interFoam,
        endTime=$(echo "$(foamListTimes -processor -latestTime -withZero | tail -1) + 2.0" | bc -l),
        deltaT=0.000001,
...
runApplication -a foamDictionary  -entry ddtSchemes/default -set Euler system/fvSchemes
runApplication -a foamDictionary  -entry relaxationFactors/fields/p -set 1 system/fvSolution
runApplication -a foamDictionary  -entry relaxationFactors/equations/U -set 1 system/fvSolution
# for directory in processor* ; do
#     rm ${directory}/constant/polyMesh/sets/protectedCells
#     pwd
# done
# runParallel -o    interFoam
...
# runParallel -a     foamDictionary -entry boundaryField/right -set\
#     "{
...
runParallel -a    foamDictionary -entry boundaryField/right/type -set "zeroGradient" $(foamListTimes -processor -latestTime | tail -1)/p_rgh
...
runParallel -a    foamDictionary  $(foamListTimes -processor -latestTime | tail -1)/uniform/time -set\
    "
        deltaT=0.00000001,
        deltaT0=0.00000001
    "
